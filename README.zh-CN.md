...
客户端也可以在未发送 `initialize` 的情况下直接请求 `tools/list`、`tools/call` 或 `server/discover`。`server/discover` 由 wrapper 自己回答，不会启动真实 MCP；如果后续请求需要访问 legacy upstream，wrapper 会在内部完成 upstream 初始化。
...
OAuth 凭据会和远程 MCP 配置绑定。运行时和 `setup` 都会校验本地凭据是否匹配当前 server URL；如果配置了 `oauth_client_id`、`oauth_resource`、`oauth_scopes`，这些字段也必须匹配。修改这些配置后，需要重新执行 `lazy-mcp-wrapper auth login <name> --config <client-mcp-config>`。
...
`upstream_protocol_mode` 用来控制 wrapper 和远程 HTTP upstream 的协议模式：

| 值 | 说明 |
| --- | --- |
| `auto` | 默认值。保持当前兼容行为，会初始化 upstream。 |
| `legacy` | 强制使用 legacy upstream 初始化。 |
| `stateless` | 仅支持 native `streamable-http`，会跳过 upstream `initialize`。SDK/OAuth 远程 MCP 暂不支持该模式。 |
...
这里的 `session` 是 `lazy-mcp-wrapper` 内部的生命周期隔离策略，不是 MCP 2026 stateless 方向要移除的协议层 session。
...
只有在你希望检测到变化后自动执行 `setup update --yes` 时，才使用 `--apply`：
...
`reload` 只支持通过 `--daemon-config` 启动的 daemon，会重新读取 daemon 配置文件。未变化的 wrapper config 会保留原 proxy、真实 MCP 进程和运行时统计；只有新增、删除或配置变化的项会被替换。手动 `daemon --config ...` 模式没有可重载源，会返回明确错误。默认情况下，如果存在活跃 client，reload 会返回 busy；使用 `--graceful` 会让新 client 使用新代理，旧 client 继续使用旧代理直到断开；使用 `--force` 会立即强制替换已变化/已删除的代理并关闭旧真实 MCP 进程。

## 缓存

`tools/list` 默认启用缓存。缓存位置默认在系统用户缓存目录下，也可以通过 `cache_dir` 指定。收到 `notifications/tools/list_changed` 时，wrapper 会先失效对应缓存，再把通知转发给客户端。

如果 upstream 的 `tools/list` 结果包含 `ttlMs`，缓存会按该 TTL 过期。`cacheScope: "session"` 和 `cacheScope: "private"` 的结果不会写入本地缓存，避免跨 client 复用私有工具元数据。
