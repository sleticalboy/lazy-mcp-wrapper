...
这类 MCP 使用 `sharing: "session"`，共享 daemon 入口，但不共享真实 MCP 进程。
...
- 支持 `sharing: "shared"` 和 `sharing: "session"` 策略。
- 支持共享 `context7`。
- 支持共享 `mastergo-magic-mcp`。
- 支持 `playwright` 通过 `sharing: "session"` 进入 daemon client 模式。
...
`sharing: "session"` 的设计见 [Session Sharing Design](./session-sharing.md)。
